...
#' @name read_eos_data-deprecated
#' @keywords internal
#'
NULL

#' @title read_eos_data is Deprecated
#' @description \code{read_eos_data} was deprecated in NVIdb v0.13.0 released
...
read_eos_data <- function(eos_table,
  ...
  # DEPRECATED ----
  .Deprecated(new = "read_eos_data",
              package = "NVIdb",
              msg = paste("'read_eos_data' is replaced by
                          'NVIpjsr::read_eos_data'"))

  if (isTRUE(NVIcheckmate::check_package(x = "NVIpjsr", type = "installed"))) {
    select_statement <- NVIpjsr::read_eos_data(eos_table = eos_table,
                                               from_path = from_path,
                                               year = year,
