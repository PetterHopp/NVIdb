...
#'
#'     The function uses a premade translation tables that is generated from information in PJS adresseregister. The translation table
#'     is updated when we know there is a need.
#'
...
add_poststed <- function(data,
  ...
  NVIcheckmate::assert_names(unname(new_column), 
                             type = "named", 
                             subset.of = colnames(translation_table), 
                             comment = paste0("The values for new_column must be column names in the translation table '",
                                              deparse(substitute(translation_table)),
                                              "' but '",
                                              unname(new_column), 
                                              "' are not column names in the translation table"),
                             add = checks)
  ...
  NVIcheckmate::assert_names(names(new_column), 
                             ...
                             comment = paste0("You cannot give the new column the same name as the code_column '",
                                              names(code_column),
                                              "' in the data '",
                                              deparse(substitute(data)), "`"),
                             add = checks)
