...
Several views retrieve data from several of the PJS-levels as in this example. By selecting unique rows from Specific PJS-levels, the number of rows in the data May be reduce considerably. You may select variables from specified PJS-levels by using the function \code{choose_PJS_levels}. The function accepts the levels as input and you do not have to specify the variables. The function will only work as intended if the column names have been standardised using  \code{standardize_PJS}. See help for 
\code{choose_PJS_levels} for further options. 
```{r, include = TRUE}
...
To translate PJS codes into descriptive text you may use the function add_PJS_code_description. The function needs arguments like the data, the translation table, the name  of the columns with the codes, the name of the resulting columns with the descriptive text and the type of the code that should be translate. There are addition al arguments to position the resulting columns and other, see help for add_PJS_code_description. When the column names have been standardised as done above, you can simplify the input arguments code_column and new_column. See help for how to do it with non-standard names. 
