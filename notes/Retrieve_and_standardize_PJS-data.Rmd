...
We can use build_query_hensikt to generate the select statement to retrieve all observations from one (or more) years and the hensikter that you give in the input statement. The function generates the sql syntax for selecting observations from the views v2_sak_res_eier and sakskonklusjon in journalrapp. These are used as input to the query sent to the database.
