...
To retrieve the PJS data, you login to the database journalrapp to established a data base connection, query the database and close the database connection. You can use login_PJS (equals login("PJS")) for the login. You will then enter the username and password, but the other specifications of the data base connection are not necessary to input. If you often login to PJS, you may store the credentials (username and password) in your user profile (at the current PC) with set_credentials_PJS. Do once These will be automatically found by login. Must be repeated when password to PJS changes or if you use another PC. To avoid being asked for credentials when running batch scripts, store your credentials with set_credentials_PJS and use login_by_credentials_PJS.

The query can be sent PJS by RODBC::sqlQuery. In this example we only select observations using the view v2_sak_res_eier and not from sakskonklusjon. The argument as.is =TRUE ensures that lading zeros are kept in character variables with only numbers.

After the wanted queries have been run, you should tidy up closing the data base connection. 
