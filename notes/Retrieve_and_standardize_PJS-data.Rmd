...
## Overview
The purpose of this vignette is to give guidelines for retrieving and standardising data from PJS. The vignette shows how functions from `NVIdb` can be used to facilitate this. Thereby, one should be able to make data sets from PJS with standard column names and standard classes for the variables to make it easier to share scripts for further management and analysis of PJS data. 
...
We retrieve data from PJS though views in the data base journalrapp. There are views available for different purposes. If we need all data for a disease, we usually use the view "v2_sak_m_res" that combines information from all levels from sak to result and in addition includes konklusjon and eier_lokalitet. 

### Make selection statement
In this example we select all saker with resultater and konklusjoner related to surveillance for paratuberkulosis in different species for a single year. To achieve this we select

- all saker with hensikt for paratuberkulosis surveillance. 

There are four main hensikter, each for one species (group of species). 
```{r, include = TRUE}
...
We can use build_query_hensikt to generate the sql syntax for selecting observations from the views v2_sak_res_eier and sakskonklusjon in journalrapp. The select statement retrieves all observations from one (or more) years AND the hensikter with or without sub-hensikter that you give in the input statement. The select statement is used as input to the query sent to the database.
```{r, include = TRUE}
...
### Login to PJS
To retrieve the PJS data, you log in to the database journalrapp to established a data base connection, query the database and close the database connection. You can use `login_PJS` (equals `login("PJS")`) for the log in. You will then enter the username and password, but the other specifications of the data base connection are not necessary to input. If you often login to PJS, you may store the credentials (username and password) in your user profile (at the current PC) with `set_credentials_PJS`. Do once These will be automatically found by `login`. Must be repeated when password to PJS changes or if you use another PC. To avoid being asked for credentials when running batch scripts, store your credentials with `set_credentials_PJS` and use `login_by_credentials_PJS`.
```{r, include = TRUE}
# READ DATA FROM PJS ----
...
### Read PJS data
The query can be sent PJS by `RODBC::sqlQuery`. In this example we only select observations using the view "v2_sak_res_eier" and not from the view "sakskonklusjon". The argument `as.is =TRUE` ensures that lading zeros are kept in character variables with only numbers.
