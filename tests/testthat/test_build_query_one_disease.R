...
test_that("build query PD", {
  ...
  correct_result <-  paste("SELECT *",
                           ...
                           "konkl_analyttkode in ('1502010235') OR konkl_analyttkode LIKE '01220104%' OR",
                           "analyttkode_funn in ('1502010235') OR analyttkode_funn LIKE '01220104%' )")
  ...
  correct_result <-  paste("SELECT v_sakskonklusjon.*, sak.mottatt_dato, sak.uttaksdato, sak.sak_avsluttet,",
                           ...
                           "WHERE sak.aar = 2020 AND ( analyttkode in ('1502010235') OR analyttkode LIKE '01220104%' )")
...
test_that("build query CMS", {
  ...
  correct_result <-  paste("SELECT *",
                           "FROM v2_sak_m_res",
                           "WHERE aar = 2020 AND",
                           "(  metodekode in ('070183') OR",
                           "konkl_analyttkode in ('15020804') OR konkl_analyttkode LIKE '012601%' OR",
                           "analyttkode_funn in ('15020804') OR analyttkode_funn LIKE '012601%' )")
  ...
  correct_result <-  paste("SELECT v_sakskonklusjon.*, sak.mottatt_dato, sak.uttaksdato, sak.sak_avsluttet,",
                           ...
                           "v_sakskonklusjon.ansvarlig_seksjon = sak.ansvarlig_seksjon AND",
                           "v_sakskonklusjon.innsendelsesnummer = sak.innsendelsesnummer)",
                           "WHERE sak.aar = 2020 AND ( analyttkode in ('15020804') OR analyttkode LIKE '012601%' )")
...
test_that("build query Gjellemykose", {
  ...
  correct_result <-  paste("SELECT *",
                           "FROM v2_sak_m_res",
                           "WHERE aar = 2020 AND",
                           "(   konkl_analyttkode in ('0702060406') OR analyttkode_funn in ('0702060406') )")
  ...
  correct_result <-  paste("SELECT v_sakskonklusjon.*, sak.mottatt_dato, sak.uttaksdato, sak.sak_avsluttet,",
                           ...
                           "v_sakskonklusjon.ansvarlig_seksjon = sak.ansvarlig_seksjon AND",
                           "v_sakskonklusjon.innsendelsesnummer = sak.innsendelsesnummer)",
                           "WHERE sak.aar = 2020 AND ( analyttkode in ('0702060406') )")
